...
impl Repository for PropertySurrealDbRepository {
    ...
    async fn get_by_id(&self, id: Self::Id) -> RepositoryResult<Self::Entity> {
        ...
        let op_user: Option<PropertyResponseDTO> =
            self.db.select(("propertys", uuid)).await.unwrap();

        op_user
            .map(PropertyEntity::from)
    ...
    async fn create(&self, entity: Self::Entity) -> RepositoryResult<Self::Entity> {
        let user_query: PropertyQueryDTO = PropertyQueryDTO::from(&entity);

        let op_user: Option<PropertyResponseDTO> = self
            .db
            .create(("propertys", entity.id.0))
            .content(user_query)
            .await
            .map_err(|e| {
                println!("{e:?}");
    ...
    async fn update(&self, new_entity: Self::Entity) -> RepositoryResult<Self::Entity> {
        ...
        let user_query: PropertyQueryDTO = PropertyQueryDTO::from(&new_entity);
