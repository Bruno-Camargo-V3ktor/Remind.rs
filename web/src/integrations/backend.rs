...
impl Backend {
    ...
    pub async fn delete_property(&self, token: Token, id: String) -> Result<(), ErrorInfos> {
        let response = self
            .client
            .delete(format!("{BASE_URL}/propertys/{id}"))
            .header("Authorization", token.0)
            .send()
            .await
            .map_err(|e| {
    ...
    pub async fn create_note(
        &self,
        token: Token,
        created: CreateNoteDTO,
    ) -> Result<Note, ErrorInfos> {
        let response = self
            .client
    ...
    pub async fn delete_note(&self, token: Token, id: String) -> Result<(), ErrorInfos> {
        let response = self
            .client
